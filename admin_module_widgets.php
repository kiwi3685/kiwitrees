...
define('WT_SCRIPT_NAME', 'admin_module_widgets.php');
...
$modules=WT_Module::getActiveWidgets(WT_GED_ID, WT_PRIV_HIDE);
...
if ($action=='update_mods' && WT_Filter::checkCsrf()) {
	foreach ($modules as $module_name=>$module) {
		foreach (WT_Tree::getAll() as $tree) {
			$access_level = safe_POST("widgetaccess-{$module_name}-{$tree->tree_id}", WT_REGEX_INTEGER, $module->defaultAccessLevel());
			WT_DB::prepare(
		...
		$order = safe_POST('widgetorder-'.$module_name);
		WT_DB::prepare(
			...
			<tbody>
				...
				foreach ($modules as $module_name=>$module) {
					...
					<tr class="sortme">
						...
						<td><input type="text" size="3" value="<?php echo $order; ?>" name="widgetorder-<?php echo $module->getName(); ?>"></td>
						<td>
							<table class="modules_table2">
								<?php
								foreach (WT_Tree::getAll() as $tree) {
									$varname = 'widgetaccess-'.$module_name.'-'.$tree->tree_id;
