...
define('KT_SCRIPT_NAME', 'famlist.php');
...
echo '
	<div id="famlist-page">
		<h2 class="center">' . KT_I18N::translate('Families') . $surnameheader . '</h2>';

		// Print a selection list of initial letters
		$list=array();
		foreach (KT_Query_Name::surnameAlpha($show_marnm, true, KT_GED_ID) as $letter=>$count) {
			switch ($letter) {
			case '@':
				$html=$UNKNOWN_NN;
				break;
		...
		if (!$SEARCH_SPIDER) {
			...
			if ($show=='surn') {
				...
				if ($count<get_gedcom_setting(KT_GED_ID, 'SUBLIST_TRIGGER_I')) {
					...
					$givn_initials=KT_Query_Name::givenAlpha($surname, $alpha, $show_marnm, true, KT_GED_ID);
				...
				if ($show=='indi') {
					echo format_fam_table(
						KT_Query_Name::families($surname, $alpha, $falpha, $show_marnm, KT_GED_ID)
					);
				}
			}
		}
