 ...
 */
class WT_Census_CensusColumnChildrenDied extends WT_Census_AbstractCensusColumn implements WT_Census_CensusColumnInterface {
	/**
		...
		foreach ($individual->getSpouseFamilies() as $family) {
			foreach ($family->getChildren() as $child) {
				if (
					...
					WT_Date::Compare($child->getDeathDate(), $this->date()) < 0
				) {
					$count++;
				}
			}
