<?php
 // Controller for the relationship chart
 //
...
require_once WT_ROOT.'library/algorithm/dijkstra.php';
...
class WT_Controller_Relationship extends WT_Controller_Page {
	 ...
	 * @return string[][]
	 */
	public function calculateRelationships(WT_Person $individual1, WT_Person $individual2, $recursion, $ancestor = false) {
		$rows = WT_DB::prepare(
			"SELECT l_from, l_to FROM `##link` WHERE l_file = :tree_id AND l_type IN ('FAMS', 'FAMC')"
		)->execute(array(
			'tree_id' => $individual1->getGedId(),
		))->fetchAll();

		// Optionally restrict the graph to the ancestors of the individuals.
		if ($ancestor) {
			$ancestors = $this->allAncestors($individual1->getXref(), $individual2->getXref(), $individual1->getGedId());
			$exclude   = $this->excludeFamilies($individual1->getXref(), $individual2->getXref(), $individual1->getGedId());
		} else {
			$ancestors = array();
			$exclude   = array();
		}
		...
		$dijkstra = new WT_Dijkstra($graph);
