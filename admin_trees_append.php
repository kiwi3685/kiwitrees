...
define('WT_SCRIPT_NAME', 'admin_trees_append.php');
...
$controller
	->requireManagerLogin()
	->setPageTitle(WT_I18N::translate('Append family trees'))
	->pageHeader();
...
if ($ged1_id && $ged2_id != $ged1_id ) {
	...
	if ($xrefs) {
		...
		'</p>',
		'<p class="error">',
		WT_I18N::translate('You must renumber the records in one of the trees before you can combine them.'),
		'</p>';
	} else {
		...
		WT_DB::exec("COMMIT");
		echo '<p>', WT_I18N::translate('The family trees were combined successfully.'), '</p>';
	}
} else {
	...
	/* I18N:  Copy all the records from [family tree 1] into [family tree 2] */ 'Copy all the records from %1$s into %2$s',
		select_edit_control('ged1_id', WT_Tree::getIdList(), '', null),
		select_edit_control('ged2_id', WT_Tree::getIdList(), '', null)
	), '</p>';
