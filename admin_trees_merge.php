...
if ($action != 'choose') {
	if ($gid1 == $gid2 && $GEDCOM == $ged2) {
		$action='choose'; ?>
		<span class="error"><?php echo WT_I18N::translate('You entered the same IDs.  You cannot merge the same records.'); ?></span>
	<?php } else {
		...
		if (empty($gedrec1)) { ?>
			<span class="error"><?php echo WT_I18N::translate('Unable to find record with ID'); ?>:</span> <?php echo $gid1; ?>, <?php echo $ged1;
			$action = 'choose';
		} elseif (empty($gedrec2)) { ?>
			<span class="error"><?php echo WT_I18N::translate('Unable to find record with ID'); ?>:</span> <?php echo $gid2; ?>, <?php echo $ged2;
			$action = 'choose';
		} else {
			$type1 = '';
			...
			if (!empty($type1) && ($type1!=$type2)) { ?>
				<span class="error"><?php echo WT_I18N::translate('Records are not the same type.  Cannot merge records that are not the same type.'); ?></span>
				<?php
				$action = 'choose';
			} else {
				$facts1		= array();
				...
				if ($action == 'select') { ?>
					<div id="merge2">
						...
						<form method="post" action="admin_trees_merge.php">
								...
								<?php foreach ($facts1 as $i=>$fact1) {
									foreach ($facts2 as $j=>$fact2) {
										if (utf8_strtoupper($fact1['subrec']) == utf8_strtoupper($fact2['subrec'])) {
											...
											$equal_count++; ?>
											<tr>
												<td><?php echo WT_I18N::translate($fact1['fact']); ?>
							...
							<br>
							<?php echo WT_I18N::translate('The following facts did not match.  Select the information you would like to keep.'); ?>
							<table>
								<tr>
									<th>
										<?php echo WT_I18N::translate('Record') . ' ' . $gid1; ?>
									</th>
									<th>
										<?php echo WT_I18N::translate('Record') . ' ' . $gid2; ?>
									</th>
								</tr>
								<tr>
									<td>
				...
				<?php } elseif ($action == 'merge') { ?>
					<div id="merge3">
						...
						<p>
							<?php echo WT_I18N::translate('Record %s successfully updated.', '<a href="' . $rec->getHtmlUrl() . '">' . $rec->getXref() . '</a>' ); ?>
						</p>
						<?php $fav_count = update_favorites($gid2, $gid1);
						if ($fav_count > 0) { ?>
							<p>
								<?php echo $fav_count . ' ' . WT_I18N::translate('favorites updated.'); ?>
							<p>
						<?php } ?>
					</div>
				<?php }
	...
	<div id="merge">
		...
		<form method="post" name="merge" action="admin_trees_merge.php">
			...
			<table>
				<tr>
					<td>
						<?php echo WT_I18N::translate('Merge To ID:'); ?>
					</td>
					<td>
						<select
							name="ged"
							...
							<?php echo count(WT_Tree::getAll()) == 1 ? 'style="width:1px; visibility:hidden;"' : ''; ?>
						>
						...
						<a href="#" onclick="iopen_find(document.merge.gid1, document.merge.ged);" tabindex="6" class="icon-button_indi" title="<?php echo WT_I18N::translate('Find an individual'); ?>"></a>
						<a href="#" onclick="fopen_find(document.merge.gid1, document.merge.ged);" tabindex="8" class="icon-button_family" title="<?php echo WT_I18N::translate('Find a family'); ?>"></a>
						<a href="#" onclick="sopen_find(document.merge.gid1, document.merge.ged);" tabindex="10" class="icon-button_source" title="<?php echo WT_I18N::translate('Find a source'); ?>"></a>
					</td>
				</tr>
				<tr>
					<td>
						<?php echo WT_I18N::translate('Merge from ID:'); ?>
					</td>
					<td>
						<select
							name="ged2"
						...
						>
							<?php foreach (WT_Tree::getAll() as $tree) { ?>
								<option value="<?php echo $tree->tree_name_html; ?>"
									<?php if (empty($ged2) && $tree->tree_id == WT_GED_ID || !empty($ged2) && $ged2 == $tree->tree_name) { ?>
										selected="selected"
									<?php } ?>
						...
						<a href="#" onclick="iopen_find(document.merge.gid2, document.merge.ged2);" tabindex="7" class="icon-button_indi" title="<?php echo WT_I18N::translate('Find an individual'); ?>"></a>
						<a href="#" onclick="fopen_find(document.merge.gid2, document.merge.ged2);" tabindex="9" class="icon-button_family" title="<?php echo WT_I18N::translate('Find a family'); ?>"></a>
						<a href="#" onclick="sopen_find(document.merge.gid2, document.merge.ged2);" tabindex="11" class="icon-button_source" title="<?php echo WT_I18N::translate('Find a source'); ?>"></a>
					</td>
				</tr>
			</table>
			<input type="submit" value="<?php echo WT_I18N::translate('next'); ?>" tabindex="3">
		</form>
	</div>
	<?php
}
