...
class gedcom_stats_KT_Module extends KT_Module implements KT_Module_Block {
	...
	public function getBlock($block_id, $template=true, $cfg=null) {
		...
		if ($stat_indi) {
			$content.='<tr><td class="facts_label">'.KT_I18N::translate('Individuals').'</td><td class="facts_value stats_value"><a href="'."indilist.php?surname_sublist=no&amp;ged=".KT_GEDURL.'">'.$stats->totalIndividuals().'</a></td></tr>';
			$content.='<tr><td class="facts_label">'.KT_I18N::translate('Males').'</td><td class="facts_value stats_value">'.$stats->totalSexMales().'<br>'.$stats->totalSexMalesPercentage().'</td></tr>';
			$content.='<tr><td class="facts_label">'.KT_I18N::translate('Females').'</td><td class="facts_value stats_value">'.$stats->totalSexFemales().'<br>'.$stats->totalSexFemalesPercentage().'</td></tr>';
		}
		if ($stat_surname) {
			$content .= '<tr><td class="facts_label">'.KT_I18N::translate('Total surnames').'</td><td class="facts_value stats_value"><a href="indilist.php?show_all=yes&amp;surname_sublist=yes&amp;ged='.KT_GEDURL.'">'.$stats->totalSurnames().'</a></td></tr>';
		}
		if ($stat_fam) {
			$content .= '<tr><td class="facts_label">'.KT_I18N::translate('Families').'</td><td class="facts_value stats_value"><a href="famlist.php?ged='.KT_GEDURL.'">'.$stats->totalFamilies().'</a></td></tr>';
		}
		if ($stat_sour) {
			$content .= '<tr><td class="facts_label">'.KT_I18N::translate('Sources').'</td><td class="facts_value stats_value"><a href="sourcelist.php?ged='.KT_GEDURL.'">'.$stats->totalSources().'</a></td></tr>';
		}
		if ($stat_media) {
			$content .= '<tr><td class="facts_label">'.KT_I18N::translate('Media objects').'</td><td class="facts_value stats_value"><a href="medialist.php?ged='.KT_GEDURL.'">'.$stats->totalMedia().'</a></td></tr>';
		}
		if ($stat_repo) {
			$content .= '<tr><td class="facts_label">'.KT_I18N::translate('Repositories').'</td><td class="facts_value stats_value"><a href="repolist.php?ged='.KT_GEDURL.'">'.$stats->totalRepositories().'</a></td></tr>';
		}
		if ($stat_events) {
			$content .= '<tr><td class="facts_label">'.KT_I18N::translate('Total events').'</td><td class="facts_value stats_value">'.$stats->totalEvents().'</td></tr>';
		}
		if ($stat_users) {
		...
		if ($stat_first_birth) {
			$content .= '<tr><td class="facts_label">'.KT_I18N::translate('Earliest birth year').'</td><td class="facts_value stats_value">'.$stats->firstBirthYear().'</td>';
			if (!$block) {
		...
		if ($stat_last_birth) {
			$content .= '<tr><td class="facts_label">'.KT_I18N::translate('Latest birth year').'</td><td class="facts_value stats_value">'.$stats->lastBirthYear().'</td>';
			if (!$block) {
		...
		if ($stat_first_death) {
			$content .= '<tr><td class="facts_label">'.KT_I18N::translate('Earliest death year').'</td><td class="facts_value stats_value">'.$stats->firstDeathYear().'</td>';
			if (!$block) {
		...
		if ($stat_last_death) {
			$content .= '<tr><td class="facts_label">'.KT_I18N::translate('Latest death year').'</td><td class="facts_value stats_value">'.$stats->lastDeathYear().'
	</td>';
			if (!$block) {
		...
		if ($stat_long_life) {
			$content .= '<tr><td class="facts_label">'.KT_I18N::translate('Person who lived the longest').'</td><td class="facts_value stats_value">'.$stats->LongestLifeAge().'</td>';
			if (!$block) {
		...
		if ($stat_avg_life) {
			$content .= '<tr><td class="facts_label">'.KT_I18N::translate('Average age at death').'</td><td class="facts_value stats_value">'.$stats->averageLifespan().'</td>';
			if (!$block) {
				$content .= '<td class="facts_value">'.KT_I18N::translate('Males').':&nbsp;'.$stats->averageLifespanMale();
				$content .= '&nbsp;&nbsp;&nbsp;'.KT_I18N::translate('Females').':&nbsp;'.$stats->averageLifespanFemale().'</td>';
			}
			$content .= '</tr>';
		}

		if ($stat_most_chil && !$block) {
			$content .= '<tr><td class="facts_label">'.KT_I18N::translate('Family with the most children').'</td><td class="facts_value stats_value">'.$stats->largestFamilySize().'</td>';
			if (!$block) {
		...
		if ($stat_avg_chil) {
			$content .= '<tr><td class="facts_label">'.KT_I18N::translate('Average number of children per family').'</td><td class="facts_value stats_value">'.$stats->averageChildren().'</td>';
			if (!$block) {
		...
		if ($show_common_surnames) {
			...
			if (count($surnames)>0) {
				$content .= '<p><b>'.KT_I18N::translate('Most Common Surnames').'</b></p>';
	...
	public function configureBlock($block_id) {
		if (KT_Filter::postBool('save') && KT_Filter::checkCsrf()) {
			...
			set_block_setting($block_id, 'stat_other',           KT_Filter::postBool('stat_other'));
			...
			set_block_setting($block_id, 'stat_link',            KT_Filter::postBool('stat_link'));
			exit;
		}
		...
		$show_common_surnames= get_block_setting($block_id, 'show_common_surnames', true);
		echo '<tr><td class="descriptionbox wrap width33">';
		echo KT_I18N::translate('Show common surnames?');
		echo '</td><td class="optionbox">';
